...
* job_FCY1_300_final_25-12-23.bash -- To run Reads_analysis.py on all samples of the 300 paired-end sequencing run.
* job_FCY1_150_F3F4_25-12-23.bash -- To run Reads_analysis.py on all F3F4 (promoter) sequencing libraries from the 150 paired-end sequencing run.
* job_FCY1_150_CDS_25-12-23.bash -- To run Reads_analysis.py on all CDS (FCY1) sequencing libraries from the 150 paired-end sequencing run. This only includes a subset of cultures, in which a set of previously characterized FCY1 mutations had been added as controls.
* compile_FCY1_300_25-12-23.bash -- To compile selection coefficients (compile_s_data.py) from the 300 paired-end run in one single file.
* compile_FCY1_150_F3F4_25-12-23.bash -- To compile selection coefficients (compile_s_data.py) from the 150 paired-end F3F4 run in one single file.
* compile_FCY1_150_CDS_25-12-23.bash -- To compile selection coefficients (compile_s_data.py) from the 150 paired-end CDS run in one single file.
* freqs_FCY1_300_25-12-23.bash -- To compile WT-relative mutant frequencies (compile_mut_freqs.py) from the 300 paired-end run in one single file.
* freqs_FCY1_150_F3F4_25-12-23.bash -- To compile WT-relative mutant frequencies (compile_mut_freqs.py) from the 150 paired-end F3F4 run in one single file.
* freqs_FCY1_150_CDS_25-12-23.bash -- To compile WT-relative mutant frequencies (compile_mut_freqs.py) from the 150 paired-end CDS run in one single file.
...
2. **Aggregate_s_coeffs.ipynb** (Supplementary Figs 8-10) -- Removal of outliers in selection coefficients and statistical analyses to identify significant fitness differences.
...
4. **Fig4.ipynb** (Fig 3; Extended Data Figs 4-5) -- Generation of Fig. 3 from the manuscript.
...
6. **Sort_calibration.ipynb** (Extended Data Figs 7-8 and Supplementary Fig 12) -- Validation and calibration of sort-seq using cytometry measurements for individually reconstructed validation mutants.
7. **Fig5.ipynb** (Fig 4) -- Generation of Fig. 4 from the manuscript.
8. **FCY1_fit_function_with_valid_Reviews.ipynb** (Extended Data Fig 9) -- Fitting of expression-fitness curves and selection coefficients calculations for reconstructed validation mutants.
9. **Dose-response_curves.ipynb** (Extended Data Figs 2 and 10) -- Fitting of dose-response curves, comparisons of fitness functions and inference of the expected fitness effects distribution 
10. **Fig6_Reviews.ipynb** (Fig 5) -- Generation of Fig 5 from the manuscript.
11. **Sim_muts_resist.ipynb** (Supplementary Figs 4 and 7) -- Simulations of competition experiments with "invisible" resistant mutants.
12. **Fig3.ipynb** (Supplementary Fig 1) -- Generation of Supplementary Fig 1 from the manuscript.
13. **S1Fig.ipynb** (Extended Data Fig 2) -- Generation of Extended Data Fig 1 from the manuscript.
14. **S2Fig.ipynb** (Supplementary Fig 2) -- Generation of Supplementary Fig 2 from the manuscript.
15. **S14Fig.ipynb** (Extended Data Fig 6) -- Generation of Extended Data Fig 6 from the manuscript.
17. **S5Fig.ipynb** (Supplementary Fig 3) -- Generation of Supplementary Fig 3 from the manuscript.
18. **S7S8Figs.ipynb** (Supplementary Figs 5,6) -- Generation of Supplementary Figs 5 and 6 from the manuscript.
19. **Gating_flow_cytometry.ipynb** (Supplementary Figs 11 and 13) -- Generation of supplementary figures displaying gating strategies used for FACS and flow cytometry experiments. 
20. **FCY1_CSM_selection.ipynb** (Extended Data Fig 3) -- Script which was used to choose codon positions from the FCY1 DMS (Després et al., 2022) to include as controls in the bulk competition experiment.
21. **FCY1_abun_quantile.ipynb** -- Very short script to compare Fcy1 abundance to that of other yeast enzymes.
22. **Read_depth.ipynb** -- Very short script looking at the sequencing coverage of mutants in the bulk competition experiment.
23. **Sort-seq_validation.ipynb** -- Short script to validate the analysis approach used for the sort-seq experiment.
